...
const paypal_client_id= 'ASSC2FNnCaWucsVofdSN5NaOTTptrjvAlLBm7KapBpnXElPkYwNfZ3wT5J-pIDDFFlUHh7yS4uwmJHzG';
const paypal_secret = 'EEG8Str3EDdiKO2P4yNYZh1jGhPuKvBzNq7gMu_FrdAx3Tl3ggzjDhzxkYCwPuNYXKS72p7KdKiLZAW5';
                ...
                admin.database().ref('bookings').child(order_id).once('value', snapshot => {
                  if(snapshot.val()){
                      ...
                      if(order_id.startsWith("wallet")){
                        addToWallet(order_id.split("-")[1], amount, order_id, transaction_id);
                        res.redirect(`/success?order_id=${order_id}&amount=${amount}&transaction_id=${transaction_id}`);
                      }else{
                        res.redirect('/cancel');
                      }
